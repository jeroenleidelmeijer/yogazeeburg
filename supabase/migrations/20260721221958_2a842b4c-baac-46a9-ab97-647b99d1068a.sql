...
DECLARE
  proj uuid;
  a1 uuid; r1 uuid; t1 uuid;
  a2 uuid; r2 uuid; t2 uuid;
  a3 uuid; r3 uuid; t3 uuid;
  claim jsonb;
  results jsonb := '[]'::jsonb;
  nid uuid;
  step_ct int;
  art_status text;
  proj_stopped boolean;
  disp text;
BEGIN
  PERFORM set_config('request.jwt.claims','{"role":"service_role","sub":null}', true);

  -- Fixture project (unique key)
  ...
  -- T1: scheduled + automation off => configuration_blocked
  ...
  -- T2: manual trigger bypasses automation => claim head (planning=1)
  ...
  -- T3: reclaim head while locked => lock_conflict
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
  results := results || jsonb_build_object('t3_lock_conflict', claim->>'disposition' = 'lock_conflict');

  -- T4: heartbeat extends
  BEGIN
    PERFORM heartbeat_publication_run(r1,a1,t1,600);
    results := results || jsonb_build_object('t4_heartbeat', true);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t4_heartbeat', false, 't4_err', SQLERRM);
  END;

  -- T5: bad lock token rejected
  BEGIN
    PERFORM heartbeat_publication_run(r1,a1,gen_random_uuid(),300);
    results := results || jsonb_build_object('t5_bad_token', false);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t5_bad_token', true);
  END;

  -- T6: transition locked->drafting
  ...
  -- T7: invalid transition rejected
  BEGIN
    PERFORM advance_publication_run(r1,a1,t1,'drafting','publishing','x','{}'::jsonb);
    results := results || jsonb_build_object('t7_invalid_transition', false);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t7_invalid_transition', true);
  END;

  -- T8: walk to live_check
  ...
  -- T9: future published_at rejected
  BEGIN
    PERFORM complete_publication_success(r1,a1,t1,'T','test-slug','h','d','https://e.com/x', now()+interval '1 hour');
    results := results || jsonb_build_object('t9_future_publish', false);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t9_future_publish', true);
  END;

  -- T10: successful publish
  ...
  -- T12: failure retry (attempt 1) -> retry_pending
  PERFORM advance_publication_run(r2,a2,t2,'locked','drafting','claim','{}'::jsonb);
  SELECT complete_publication_failure(r2,a2,t2,'draft','retry','transient_llm_error','llm_timeout','timeout1','{}'::jsonb,30) INTO claim;
  ...
  -- T13: retry to exhaustion (max=3). One failure recorded; need 2 more.
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'retry',NULL,300) INTO claim;
  r2 := (claim->>'run_id')::uuid; a2 := (claim->>'article_id')::uuid; t2 := (claim->>'lock_token')::uuid;
  PERFORM advance_publication_run(r2,a2,t2,'locked','drafting','claim','{}'::jsonb);
  PERFORM complete_publication_failure(r2,a2,t2,'draft','retry','transient_llm_error','llm_timeout','timeout2','{}'::jsonb,NULL);

  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'retry',NULL,300) INTO claim;
  r2 := (claim->>'run_id')::uuid; a2 := (claim->>'article_id')::uuid; t2 := (claim->>'lock_token')::uuid;
  PERFORM advance_publication_run(r2,a2,t2,'locked','drafting','claim','{}'::jsonb);
  SELECT complete_publication_failure(r2,a2,t2,'draft','retry','transient_llm_error','llm_timeout','timeout3','{}'::jsonb,NULL) INTO claim;
  SELECT status::text INTO art_status FROM publication_articles WHERE id=a2;
  results := results || jsonb_build_object('t13_exhausted',
    art_status='failed' AND (claim->>'exhausted')::boolean = true);

  -- T14: sequence-head failed -> sequence_blocked
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
  results := results || jsonb_build_object('t14_sequence_blocked', claim->>'disposition' = 'sequence_blocked');

  -- Reset article 2 to planned for recovery tests (via privileged UPDATE inside SECURITY DEFINER function)
  UPDATE publication_articles SET status='planned', retry_count=0,
     last_error_category=NULL, last_error_summary=NULL WHERE id=a2;

  -- T15: stale lock in drafting -> safe recovery, then claim same article
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
  ...
  -- T16: stale lock in publishing -> recovery_blocked
  PERFORM advance_publication_run(r3,a3,t3,'locked','drafting','claim','{}'::jsonb);
  ...
  results := results || jsonb_build_object('t16_recovery_blocked',
    claim->>'disposition' = 'recovery_blocked'
    AND art_status = 'blocked'
  ...
  -- Idempotent replay
  BEGIN
    PERFORM mark_notification_result(nid,(SELECT idempotency_key FROM publication_notifications WHERE id=nid),'sent');
    results := results || jsonb_build_object('t17a_idempotent_sent', true);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t17a_idempotent_sent', false, 'err', SQLERRM);
  END;
  -- Sent -> failed rejected
  BEGIN
    PERFORM mark_notification_result(nid,(SELECT idempotency_key FROM publication_notifications WHERE id=nid),'failed',NULL,'e');
    results := results || jsonb_build_object('t17b_sent_terminal', false);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t17b_sent_terminal', true);
  END;
  -- Bad idempotency key
  BEGIN
    PERFORM mark_notification_result(nid,'wrong-key','sending');
    results := results || jsonb_build_object('t17c_bad_key', false);
  EXCEPTION WHEN OTHERS THEN
    results := results || jsonb_build_object('t17c_bad_key', true);
  END;

  -- T18: no unpublished head w/ 5 fixture articles => publish them all, then claim
  UPDATE publication_articles SET status='published', final_title='x', slug='fs'||planning_number,
        content_hash='h', deployment_id='d', live_url='https://e.com/f'||planning_number,
        published_at=now(), lock_token=NULL, locked_at=NULL, lock_expires_at=NULL, locked_by=NULL, active_run_id=NULL
   WHERE project_id=proj AND status <> 'published';
  -- Now no unpublished head; but < 180 total. Path: no head found -> stopped_noop with all_180_published,
  -- but count(published) < 180 so first check misses; fallthrough SELECT LIMIT 1 returns nothing => second branch stops.
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
  ...
  -- Rollback ALL fixture data by raising, then caller catches.
