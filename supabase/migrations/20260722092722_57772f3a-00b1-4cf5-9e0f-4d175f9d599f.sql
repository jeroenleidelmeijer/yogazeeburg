...
BEGIN
  ...
  results := results || jsonb_build_object('t12_retry_pending',
    (claim->>'run_status')='retry_pending' AND (claim->>'step_attempt')::int = 1 AND (claim->>'exhausted')::boolean = false);
  ...
  results := results || jsonb_build_object('t13_exhausted',
    ...
    AND (claim->>'step_attempt')::int = 3 AND (claim->>'run_status')='failed');
  ...
  -- t14c: different step_key on same article restarts numbering at 1
  ...
  -- t14d/t14e: force a2 to published so next head is a different article; failure history must not carry over
  UPDATE publication_articles SET status='published', final_title='x', slug='p2-'||gen_random_uuid()::text,
      content_hash='h', deployment_id='d', live_url='https://e.com/p2', published_at=now(),
      lock_token=NULL, locked_at=NULL, lock_expires_at=NULL, locked_by=NULL, active_run_id=NULL
   WHERE id=a2;
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
  a_other := (claim->>'article_id')::uuid;
  results := results || jsonb_build_object('t14d_other_article_head', a_other <> a2 AND claim->>'disposition'='claimed');
  ...
  -- t14f: recovery-safe retry continues the same (article, step) history
  -- a_other now has 1 'draft' failure. Reclaim (retry), advance, force stale lock (recovery-safe → planned), reclaim, advance, fail again → attempt 2
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'retry',NULL,300) INTO claim;
  r_other := (claim->>'run_id')::uuid; a_other := (claim->>'article_id')::uuid; t_other := (claim->>'lock_token')::uuid;
  PERFORM advance_publication_run(r_other,a_other,t_other,'locked','drafting','claim','{}'::jsonb);
  ...
  -- t16: ambiguous recovery from publishing → recovery_blocked
  UPDATE publication_articles SET status='published', final_title='x', slug='p3-'||gen_random_uuid()::text,
      content_hash='h', deployment_id='d', live_url='https://e.com/p3', published_at=now(),
      lock_token=NULL, locked_at=NULL, lock_expires_at=NULL, locked_by=NULL, active_run_id=NULL
   WHERE id=a_other;
  SELECT claim_next_publication_run((SELECT project_key FROM publication_projects WHERE id=proj),'manual',NULL,300) INTO claim;
